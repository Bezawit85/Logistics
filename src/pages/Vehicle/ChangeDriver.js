...
const ChangeDriver = () => {
  ...
  const apiOwnerDrivers = `${mainAPI}${endpoints.List.OwnerDrivers}/${ownerId}`;
  useEffect(() => {
    setError("");
    getUnAssigned();
  }, [refresh]);
  ...
  const changeDriver = async (driver) => {
    ...
    const url = `${mainAPI}${endpoints.Update.ChangeDriver}`;
    try {
  ...
  const assignDriver = async (driver) => {
    ...
    const url = `${mainAPI}${endpoints.Create.AssignDriver}`;
    try {
      const response = await fetch(url, options);
      const result = await response.json();
      console.log(result);
