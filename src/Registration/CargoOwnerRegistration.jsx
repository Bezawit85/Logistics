...
const CargoOwnerRegistration = ({isCallCenter}) => {
  const {
    handleSubmit,
    formState: { errors },
    register,
  ...
  const businessSectorapi = `${mainAPI}${endpoints.List.BusinessSector}`;
  const businessTypeapi = `${mainAPI}${endpoints.List.BusinessType}`;
  ...
  const CargoInfo = [
    ...
    const getbusinessSector = async () => {
      ...
      const AddCargoOwner = async (formData)=>{
              ...
              setLoading(true);
          axios.post(
                  `${mainAPI}${endpoints.Create.CargoRegister}`,
                  formData,
                  {
                      headers: {
