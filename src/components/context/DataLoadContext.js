...
const DataLoadContext = ({ children }) => {
  ...
  const apiAllDrivers = `${mainAPI}${endpoints.List.Users}/Drivers`;
  const apiOnRoute = `${mainAPI}${endpoints.List.DriversbyStatus}/ONROUTE`;
  const apiAssigned = `${mainAPI}${endpoints.List.DriversbyStatus}/ASSIGNED`;
  const apiUnAssigned = `${mainAPI}${endpoints.List.DriversbyStatus}/UNASSIGNED`;
  const apiPermit = `${mainAPI}${endpoints.List.DriversbyStatus}/PERMIT`;
