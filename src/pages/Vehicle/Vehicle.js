...
const Vehicle = ({ vehicleData }) => {
  const { isCallCenter } = useUserContext();
  ...
  const apiMaintaining = `${mainAPI}${endpoints.List.VehicleStatus}/MAINTAINING`;
  const apiVehicleOnRoute = `${mainAPI}${endpoints.List.VehicleStatus}/ONROUTE`;
  const apiInStock = `${mainAPI}${endpoints.List.VehicleStatus}/INSTOCK`;
  const apiParked = `${mainAPI}${endpoints.List.VehicleStatus}/PARKED`;
