...
const VehicleDetail = ({isCallCenter}) => {
  const { vehicleId } = useParams();
  const apiVehicleDetail = `${mainAPI}${endpoints.List.Vehicles}/${vehicleId}`;
  const apiVehicleStatus = `${mainAPI}${endpoints.Create.VehicleStatus}`;
  ...
  const updateVehicleInfo = async () => {
    ...
    const url = `${mainAPI}${endpoints.Update.InfoVehicle}/${vehicleId}`;
    try {
